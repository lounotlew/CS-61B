...
class Game {
    ...
    /** Pushes the current game state (before the player makes a move),
     * GAMESTATE, to the top of the stack _PREVSTATES. */
    void saveState(Game gameState) {
        _prevStates.push(new Game(gameState));
    }

    /** Removes the top of the stack _PREVSTATES and returns it. If the size
     * of _PREVSTATES is 0 (i.e. empty), then undo() has no effect. */
    void undo() {
        if (_prevStates.size() == 0) {
            System.out.println("Nothing to undo.");
            return;
        }
        copyFrom(_prevStates.pop());
    }

    /** Returns true if the stack _prevStates is empty. Else, returns false.
     * Only used for JUnit testing; does not affect game logic. */
    boolean prevStatesEmpty() {
    ...
    /** Returns the #N card from the #K Tableau pile, with the 0th card
     * being the top card. */
    Card getTableauCard(int k, int n) {
        return tableau(k).get(n);
    }
    ...
    private Card _base;
    /** A stack that stores previous game states. */
    private Stack<Game> _prevStates = new Stack<Game>();
