...
public class CompoundInterest {
    ...
    static double futureValue(double presentValue, double rate,
                              int targetYear) {
        int difference = CompoundInterest.numYears(targetYear);
        return presentValue * Math.pow(1 + rate / 100, difference);
    }
    ...
    static double futureValueReal(double presentValue, double rate,
        ...
        int difference = CompoundInterest.numYears(targetYear);
        double futureVal = CompoundInterest.futureValue(presentValue, rate, targetYear);
        return futureVal * Math.pow(1 - inflationRate / 100, difference);
    }
    ...
    static double totalSavings(double perYear, int targetYear, double rate) {
        int k = CompoundInterest.numYears(targetYear);
        double math_rate = 1 + rate / 100;
        double result = 0;
        for (int i = k; i >= 0; i -= 1) {
            result = result + perYear * Math.pow(math_rate, i);
        }
        return result;
    }
    ...
    static double totalSavingsReal(double perYear, int targetYear, double rate,
        ...
        int k = CompoundInterest.numYears(targetYear);
        double math_rate = 1 + rate / 100;
        double inflation = 1 - inflationRate / 100;
        double result = 0;
        for (int i = k; i >= 0; i -= 1) {
            result = result + perYear * Math.pow(math_rate, i) * Math.pow(inflation, i);
        }
        return result;
    }
