...
public class CompoundInterest {
    ...
    static double futureValue(double presentValue, double rate,
                              int targetYear) {
        int n = numYears(targetYear);
        double growthFactor = Math.pow((1 + rate/100), n);
        return growthFactor * presentValue;
    }
    ...
    static double futureValueReal(double presentValue, double rate,
        ...
        double realValue = futureValue(nominalValue, -inflationRate, targetYear);
        return realValue;
    }
    ...
    static double totalSavings(double perYear, int targetYear, double rate) {
        double currentValue = perYear;
        int n = numYears(targetYear);
        for (int i = 0; i < n; i++) {
            currentValue = currentValue * (1 + rate/100);
            currentValue = currentValue + perYear;
        }
        return currentValue;
    }
    ...
    static double totalSavingsReal(double perYear, int targetYear, double rate,
        ...
        double nominalSavings = totalSavings(perYear, targetYear, rate);
        return futureValue(nominalSavings, -inflationRate, targetYear);
    }
